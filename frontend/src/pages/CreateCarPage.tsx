...
interface CreateCarPageProps {
  editMode?: boolean;
  carData?: Car; // pokud editace, předáš data auta
}
...
export const CreateCarPage: React.FC<CreateCarPageProps> = ({ editMode = false, carData }) => {
  ...
  const [layout, setLayout] = useState(carData?.layout ?? 'Sedan (4 seats)')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const layoutCapacity = layout === 'Sedan (4 seats)' ? 4
        : layout === 'Coupé (2 seats)' ? 2
        : layout === 'Minivan (7 seats)' ? 7
        : 0;

      const occupiedSeats = carData?.seats?.filter(seat => seat.occupied).length ?? 0;

      if (occupiedSeats > layoutCapacity) {
          ...
          <div className="main-card">
            <div className="main-card-header text-center">
              <h1 className='text-2xl font-bold'>
                {editMode ? 'Úprava auta a jízdy' : 'Vytvoření auta a jízdy'}
              </h1>
            </div>
            <div className="main-card-body">
              ...
              >
                ...
                <div className="form-group">
                  <label htmlFor="name" className="form-label">Název auta</label>
                  <input
                    type="text"
                    placeholder="Fabián"
                ...
                <div className="form-group">
                  <label htmlFor="date" className="form-label">Kdy pojedete?</label>
                ...
                <div className="form-group text-center">
                  ...
                  >
                    {editMode ? "Uložit změny" : "Vytvořit auto"}
                  </button>
                </div>
              </form>
            </div>
